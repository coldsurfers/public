...
/**
 * props 표의 데이터 — **DS 소스의 타입에서 직접 뽑는다.** 문서에 옮겨 적지 않는다.
 *
 ...
 *
 * 화면의 표(`components/props.tsx`)와 평문 사본(`lib/llm-text.ts`)이 같은 이 함수를 문다.
 */
 ...
 * 펼치고(`boolean` 이 `false | true` 로 망가지는 걸 막는다), 원래 별칭 이름은 따로 남긴다.
 */
...
export interface PropEntry {
  name: string
  /** 표에 적을 타입. 별칭이 펼쳐졌으면 펼친 값. */
  type: string
  /** 펼쳐졌을 때의 원래 별칭 이름. 안 펼쳐졌으면 없다. */
  alias?: string
  description?: string
  defaultValue?: string
  required: boolean
...
export async function collectProps({
  ...
  return {
    entries: picked.map((entry) => ({
      ...
      alias: expanded(entry) ? entry.type : undefined,
      description: entry.description || undefined,
      defaultValue: entry.tags.find((tag) => tag.name === 'defaultValue')?.text,
      required: entry.required,
      deprecated: Boolean(entry.deprecated),
    })),
    inherited: inheritedFrom(source, typeName),
  }
}

/** 별칭이 값으로 펼쳐졌는가. 펼쳐졌을 때만 원래 이름을 따로 남긴다. */
