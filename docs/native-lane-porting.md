...
규율의 근거는 *두 구현이 갈린다* 이다. **갈릴 상대가 원리적으로 없으면 근거가 성립하지 않고,
규칙도 안 걸린다**(「공통 prop」의 판정과 같은 어법).

웹에 짝이 *아직* 없는 것과 *생길 수 없는* 것은 다르다. 판정은 한 줄이다:

> 이 표면을 웹에 만들려면 **웹에 없는 입력·자리**가 필요한가?
> · 필요하다 → 예외. native 에만 둔다
> · 아니다 → 규율대로 웹부터

| | 왜 짝이 없는가 |
| --- | --- |
| `PullToRefresh` | 당김은 **오버스크롤 제스처**다. 웹엔 스크롤 컨테이너가 경계를 넘겨 끌리는 자리가 없고, 있어도 새로고침은 브라우저 크롬의 것이다 |
| `TabBar` | 화면 하단에 고정되는 **safe-area 를 무는 네비게이션 면**이다. 웹의 하단 내비는 같은 이름을 써도 홈 인디케이터도 탭 제스처도 없어, 치수·색만 같고 계약이 다르다 |

예외로 올린 것은 **`contract/` 파일을 만들지 않는다.** 한쪽만 있으면 계약이 아니다
(「한쪽에만 있는 컴포넌트」와 같은 처리) — 치수는 컴포넌트 곁에 `X.constants.ts` 로 둔다.
나중에 웹 짝이 생기면 그때 `contract/` 로 올리는 게 계약 확장이고, 순서는 그때부터 웹부터다.

⚠️ **예외는 표면에만 준다.** `Chip` 에 라벨 슬롯을 여는 것처럼 **웹에도 있는 컴포넌트의 축**은
예외가 아니다 — 웹부터 늘린다.
