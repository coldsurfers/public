...
**본문은 [`.github/pull_request_template.md`](.github/pull_request_template.md) 를 따른다.** 섹션
다섯(무엇을 · 왜 · 검증 · 계약 · 남은 것)을 지우거나 순서를 바꾸지 않는다. 주석(`<!-- -->`)은
지침이라 읽고 채운 뒤 지운다.

- **검증엔 실제 출력을 붙인다.** "통과함" 은 증거가 아니다 — 숫자로 적는다. 안 돌린 게 있으면
  안 돌렸다고 쓴다
- **계약 체크박스는 해당 없는 줄만 지운다.** 해당되는데 확인 못 했으면 체크를 비워두고 `남은 것`
  에 적는다. 지워서 없애지 않는다
- **`gh pr create` 는 `--body` 를 주면 템플릿을 안 붙인다.** 템플릿을 읽어 채운 파일을
  `--body-file` 로 넘긴다
